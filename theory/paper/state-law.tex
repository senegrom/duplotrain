...
Nothing forbids $w(p) = p$ (a track loop from a port back to itself) or
joining two ports of the same switch; both occur in the extremal
layouts.
...
\begin{array}{c|c|c}
  ...
  4 & 0 & \Right\Right\\
  6 & 1 \text{ (left branch }0\text{)} & \Left\Right
\end{array}
\]
All four vectors of two switches appear, so $f(2) = 4 = 2^2$.  The next
flip returns to $\Left\Left$: the run cycles through the square forever.
...
\begin{proof}[Proof (rendered)]
  ...
  Section~\ref{ssec:cascade}), and arrives back at the top in the state
  it left, at time $2N-2$, entering the left branch of $N-2$.

  \emph{Phase 3 (closing the far switch).}  That trailing entry re-pins
  $N-2$ (already $\Right$), exits its stem\,---\,but the double link now
  routes through switch $N-1$: the train enters the stem of $N-1$,
  \emph{trailing entries along the way set $N-1$ to $\Right$} at time
  $2N-1$ (all $N$ switches $\Right$), and the right-branch--right-branch
  link returns it into the right branch of $N-2$ trailing from the other
  side, whose next passage flips $N-2$ back to $\Left$ at time $2N$.

  \emph{Phase 4 (the Gray oscillation).}  From here the run settles into
  a four-corner oscillation on coordinates $N-2$ and $0$: down the
  (facing, unchanged) chain, around the teardrop lobe --- flipping switch
  $0$ --- back up, around the double link --- flipping $N-2$ --- and so
  on.  One full circuit takes $2N-1$ steps and flips exactly one of the
  two coordinates per half-circuit; times $3N-1$ and $4N-1$ sample the
