...
\subsection{Arbitrary starts, by completing the start port}
\label{ssec:boundary}

A general start need not arrive over an edge: the train may simply be
placed at a port, so Theorem~\ref{thm:knownedge} does not apply directly.
Rather than shift the run forward and pay separately for the vector it
leaves behind at time zero, complete the wiring so that the hypothesis
holds of the original run.

The model admits self-links --- a port joined to itself is a reversing
dead end --- and the known-edge theorem is uniform over all raw finite
wirings, so a cap is a legitimate wiring and costs no switch.

\begin{lemma}[Capping a free port;
  \lean{Wiring.capFreePort}, \file{StateLawNAddFourSharp.lean}]
  \label{lem:cap}
  If $w$ leaves port $p$ unwired, then joining $p$ to itself yields a
  wiring $v$ that agrees with $w$ on every edge of $w$.
\end{lemma}

\begin{proof}
  Symmetry is the only condition to check.  At $p$ the new edge is its
  own reverse.  Away from $p$, an edge of $v$ is an edge of $w$, and its
  far end cannot be $p$: symmetry of $w$ would then give $w$ an edge out
  of $p$, which it has none.  Every edge of $w$ survives for the same
  reason.
\end{proof}
...
\begin{lemma}[Extension preserves live runs;
  ...
  If every edge of $w$ is an edge of $v$, then every run of $w$ that is
  still alive after $n$ steps runs identically in $v$.
\end{lemma}

\begin{proof}
  Induction on $n$.  A surviving step follows an edge of $w$, which is
  unchanged in $v$, and the tongue update reads only the entry port.  The
  statement is deliberately silent about what happens after the original
  run dies, which is the only place the new cap can be reached.
\end{proof}
...
\begin{proof}
  Let the start be at port $p$.

  If $p \ge 3N$, then the exit port of the first move lies on the same
  switch as $p$, so it too is at least $3N$ and the port bound leaves it
  unwired.  The run therefore dies at its first step, only time zero can
  be live, and the list has length at most $1 \le N+4$.

  If $p < 3N$ and $w$ wires $p$, symmetry supplies an edge into $p$ and
  Theorem~\ref{thm:knownedge} applies to the run as it stands.

  If $p < 3N$ and $p$ is unwired, cap it (Lemma~\ref{lem:cap}).  The
  capped wiring still uses only switches $0,\dots,N-1$, since the added
  edge joins $p$ to itself.  By Lemma~\ref{lem:extend} every sampled time
  is still live and carries the same restricted vector, so the sampled
  vectors remain pairwise distinct; and in the capped wiring $p$ has an
  incoming edge, namely its own.  Theorem~\ref{thm:knownedge} then bounds
  the original list, time zero included, by $N+4$.
\end{proof}
...
\begin{tabular}{@{}p{0.42\textwidth}p{0.52\textwidth}@{}}
  ...
  \file{StateLawNAddFourSharp.lean} & the known-edge theorem, the start
    capping, and the sharp $N+4$ bound\\
