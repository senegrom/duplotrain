...
\begin{lemma}[Protected pair; \file{ProtectedPairNAddFour.lean},
  ...
  and the tail adds at most two fresh vectors. For a flip reflector,
...
Packaging the closed tree gives the keystone, discharged inline in
\lean{state\_law\_N\_add\_four}: the probe
\lean{known\_edge\_N\_add\_four\_or\_changed\_contact\_or\_protected\_pair}
leaves the two residual branches, closed by
\lean{ChangedContact.all\_run\_distinct\_le\_N\_add\_four} and
\lean{knownEdgeProtectedPairNAddFourLaw}.
...
(\file{TrackThetaPointwiseCore.lean}).
