...
\begin{proposition}[Finite-state ceiling; \lean{state\_law\_two\_pow},
  \file{StateLawSmallN.lean}]\label{prop:ceiling}
...
\begin{proof}
  Send a boolean list $b_0 b_1 \cdots b_{m-1}$ to its little-endian binary
  value $\sum_i b_i 2^i$.  On lists of one fixed length this map is
  injective (the head is the value modulo~$2$, the tail its half) and takes
  values below $2^m$.  Restricted vectors have length $N$, so pairwise
  distinct vectors map to pairwise distinct naturals below $2^N$.
\end{proof}

\begin{lemma}[\lean{add\_four\_le\_two\_pow}, \file{StateLawSmallN.lean}]
  \label{lem:minside}
  $N + 4 \le 2^N$ for $N \ge 3$; so $\min(2^N, N+4) = N+4$ for $N \ge 3$
  and $= 2^N$ for $N \le 2$.
\end{lemma}

\begin{proof}
  $3 + 4 = 7 \le 8$, and doubling outgrows adding one.
\end{proof}
