import TrackQuantitativeRepairSixteen
import NoveltyAwareLassoEighteen

/-! Known-edge coefficient-18 linear bound for `N >= 2`. -/

namespace GeneralN

/-- Known-edge long-run form of the `18*N+2` bound. -/
theorem known_edge_long_run_distinct_le_eighteen
    {w : Wiring} {N e : Nat}
  ...
  · obtain ⟨A, B, stateA, stateB,
    ...
    have hlocal := manufactured_pair_protected_repair_within_sixteen
      hN hNtwo A B hAatBase hBatActivated
    rw [← hactivatedB] at hlocal
    exact two_manufacturing_journeys_then_repair_distinct_le_eighteen
