...
/-!
# Export the switch-simplicity already proved by the splice construction

`ChangedForwardMerge.spliced_lobe_reflector` internally proves that the fresh
approach is a prefix of a switch-simple selected route, but its result did not
export that certificate.  This strengthened version is the same construction
with `SwitchSimple approach` retained in the conclusion.
-/

namespace GeneralN

/-- Strengthened spliced-lobe package retaining switch-simplicity of the fresh
approach. -/
theorem ManufacturedReflector.ChangedForwardMerge.spliced_lobe_reflector_simple
    {w : Wiring} {g e : Nat}
