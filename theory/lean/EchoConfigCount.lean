...
private theorem sum_map_zero {α : Type} :
    ∀ xs : List α, (xs.map (fun _ => (0 : Nat))).sum = 0 := by
  intro xs
  induction xs with
  | nil => rfl
...
theorem entryFibreSizes_sum
  ...
  | nil =>
      intro _ _
      show (entries.map (fun q => entryFibreSize e q [])).sum = 0
      have h : ∀ q ∈ entries, entryFibreSize e q [] = 0 := by
        intro q _
        rfl
      rw [List.map_congr_left h]
      exact sum_map_zero entries
  | cons k rest ih =>
      intro hnd hcover
      ...
      have hfold : (entries.map
          (fun q => entryFibreSize e q rest)).sum = rest.length :=
        ih hnd hrest
      rw [hfold]
...
theorem entryFibre_snap_nodup
  ...
  have hpair :
    ...
    have hmapEq : ks.map (fun k => (e k, snap m e r0 cells k))
        = ks.map (configSnap m e r0 cells) := by
      apply List.map_congr_left
      intro k _
      rfl
    rw [hmapEq]
