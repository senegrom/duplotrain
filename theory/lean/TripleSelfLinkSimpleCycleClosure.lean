...
/-- Apply the trace-retaining fork to the actual outside-oriented period of a
raw self-link cycle. -/
theorem RawCycleThroughSelfLink.outside_first_revisit_trace_or_reflector
    {w : Wiring} {start : Prod Nat Tongues} {close : Nat}
    ...
    exists outside atRepeat visited,
      ...
      ((exists cycle settled,
          ...
          PhysicalTrace w (atRepeat.1, settled) cycle
            (atRepeat.1, settled) /\
          SwitchSimple cycle) \/
        exists (A : ManufacturedReflector w outside
              (3 * (R.branch / 3)))
  ...
  have hout := first_revisit_cycle_traces_or_activated_reflector w
    hrunway hexcursion hsimpleBefore hswitch hrepeat hmouth
  refine ⟨outside, (q, u), before.length, hmouth, ?_, ?_⟩
  ...
  · rcases hout with hcycle | hreflector
    ...
    · obtain ⟨A, state, hA, hbase, hstate, hback,
      ...
      exact Or.inr ⟨A, state, runway.length + 1,
        hA, by simpa using hbase, hstate, hback, by simpa using hpreserves⟩
