...
theorem known_edge_N_add_four
  ...
  · obtain ⟨lead, atRepeat, settled, htrace, hsimple, hsettled⟩ := hcycleA
    have hcover := history_then_settled_one_novelty htrace.sound
      (history := (List.range (lead.length + 1)).map (restrictedTonguesAt w N start))
      (fun k hk => List.mem_map.mpr ⟨k, List.mem_range.mpr (by omega), rfl⟩) hsettled times
    have hcount := noveltyCoverOn_distinct_count hcover hnd
    have hL := htrace.simple_length_le hN hsimple
