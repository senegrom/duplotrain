...
/-!
# Sharper theta-capture length

A theta capture is not an arbitrary sum of two complete reflector travels.
It consists of

* a prefix of one switch-simple selected route, of length at most `N`; and
* the old reflector's mouth-to-boundary return, bounded by its complete
  `2*N` travel.

Hence every capture costs at most `3*N`.  Substituting this in the theta-half
construction lowers one macro-step from `8*N` to `7*N`.
-/

namespace GeneralN

private theorem ManufacturedFlipReflector.capture_tail_le_two
    {w : Wiring} {N g e : Nat}
    (hN : ∀ p q, w.link p = some q →
      p < 3 * N ∧ q < 3 * N)
    (A : ManufacturedFlipReflector w g e) :
    A.candy.length + 2 + A.runway.length ≤ 2 * N := by
  have htravel : A.toSupported.travel ≤ 2 * N := by
    simpa [ManufacturedReflector.toSupported] using
      (ManufacturedReflector.flip A).travel_le_two_mul_switches hN
  change A.candy.length + 2 + A.runway.length ≤ 2 * N
  change 2 * A.runway.length + A.candy.length + 2 ≤ 2 * N at htravel
  omega

/-- Runway capture within `3*N`. -/
theorem manufactured_runway_theta_capture_within_three
    {w : Wiring} {N g e : Nat}
    (hN : ∀ p q, w.link p = some q →
      p < 3 * N ∧ q < 3 * N)
    (A : ManufacturedFlipReflector w g e)
  ...
  have hcaptureLe := A.capture_tail_le_two hN
...
/-- Forward-candy capture within `3*N`. -/
theorem manufactured_candy_forward_theta_capture_within_three
    {w : Wiring} {N g e : Nat}
    (hN : ∀ p q, w.link p = some q →
      p < 3 * N ∧ q < 3 * N)
    (A : ManufacturedFlipReflector w g e)
...
private theorem ManufacturedFlipReflector.arm_values_ne
    {w : Wiring} {g e : Nat}
...
/-- Reverse-candy capture within `3*N`. -/
theorem manufactured_candy_reverse_theta_capture_within_three
    {w : Wiring} {N g e : Nat}
    (hN : ∀ p q, w.link p = some q →
      p < 3 * N ∧ q < 3 * N)
    (A : ManufacturedFlipReflector w g e)
  ...
  have hnotFirst : state B.actionSwitch ≠ bval B.firstArm := by
    intro hfirst
    exact B.arm_values_ne (hfirst.symm.trans hselected)
  have hrouteSimple :=
...
/-- Orientation-free support fault with capture bounded by `3*N`. -/
theorem manufactured_support_fault_dichotomy_within_three
    {w : Wiring} {N g e : Nat}
    (hN : ∀ p q, w.link p = some q →
      p < 3 * N ∧ q < 3 * N)
    (A : ManufacturedFlipReflector w g e)
  ...
  · rcases passage with ⟨p, x⟩
    ...
    · have hpMouth : p = A.mouth := by omega
      subst p
      left
      exact manufactured_runway_theta_capture_within_three
  ...
  · rcases passage with ⟨p, x⟩
    ...
    · rcases hstem with hpStem | hxStem
      · have hpMouth : p = A.mouth := by omega
        subst p
        left
        exact manufactured_candy_forward_theta_capture_within_three
    ...
    · rcases hstem with hpStem | hxStem
      ...
      · have hxMouth : x = A.mouth := by omega
        left
        exact manufactured_candy_reverse_theta_capture_within_three
          hN A B state hA hB hreverse
            (by simpa [hxMouth] using hoccurs)

/-- One theta macro-step has length at most `7*N`. -/
theorem manufactured_theta_half_within_seven
    {w : Wiring} {N g e : Nat}
    (hN : ∀ p q, w.link p = some q →
      p < 3 * N ∧ q < 3 * N)
    (A : ManufacturedFlipReflector w g e)
  ...
  rcases manufactured_support_fault_dichotomy_within_three
