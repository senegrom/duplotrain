...
/-! Overlap-aware count for the `15*N+2` protected repair. -/

namespace GeneralN

/-- Two exact manufactured journeys followed by a `15*N+2` protected-repair
lasso expose at most `17*N+4` distinct restricted tongue vectors. -/
theorem two_manufacturing_journeys_then_repair_distinct_le_seventeen
    {w : Wiring} {N e : Nat}
    ...
    (hlocal : EventuallyPeriodicWithin w (start.1, stateB) (15 * N + 2))
    (times : List Nat)
    (hlive : ∀ k ∈ times, (stepN w k start).isSome)
    (hnd : (times.map (restrictedTonguesAt w N start)).Nodup) :
    times.length ≤ 17 * N + 4 := by
