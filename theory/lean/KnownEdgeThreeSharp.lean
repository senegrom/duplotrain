...
/-!
# Boundary-sharp known-edge coefficient-3 bound

The endpoint of the second manufacturing journey is suffix time zero.  It is
already present in the construction history, so the protected-repair suffix
contributes at most `N+3` further vectors rather than `N+4`.
-/

namespace GeneralN

/-- **Boundary-sharp known-edge coefficient-three bound:** `3*N+6`. -/
theorem known_edge_long_run_distinct_le_three_sharp
    {w : Wiring} {N e : Nat}
