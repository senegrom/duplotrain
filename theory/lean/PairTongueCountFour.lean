...
/-- Complete repair, sharpened: the switch-simple route costs at most `N`
sample positions and the repaired pair `8*N+4` vectors, so the branch
exposes at most `9*N+4` distinct restricted tongue vectors. -/
theorem ManufacturedReflector.completed_route_with_pair_support_distinct_le_nine_succ_four
    {w : Wiring} {N g e : Nat}
    (hN : ∀ p q, w.link p = some q →
      p < 3 * N ∧ q < 3 * N)
    (A : ManufacturedReflector w g e)
  ...
  have hpreNodup : preTimes.Nodup := by
    dsimp [preTimes]
    exact pairfour_nodup_filter_nat _ htimesNodup
  ...
  have htailNodup :
    ...
    dsimp [tailTimes]
    exact pairfour_nodup_map_filter _ hnd
  have hshiftedNodup :
  ...
  have hshiftedLive : ∀ d ∈ shifted,
    ...
    exact hkLive
  have htailBound : shifted.length ≤ 8 * N + 4 :=
    manufactured_pair_reached_tongue_vector_count_eight_succ_four
  ...
  have htailLength : tailTimes.length = shifted.length := by
    simp [shifted]
  have hpartition := pairfour_lt_ge_partition L times
