...
/-- Direct distinct-vector count for an intersecting flip/stay pair. -/
theorem manufactured_flip_then_stay_distinct_le_two
    {w : Wiring} {N g e : Nat}
    (A : ManufacturedFlipReflector w g e)
    (B : ManufacturedStayReflector w e g)
    (state : Tongues)
    ...
    times.length ≤ 2 := by
  have hphase := manufactured_flip_then_stay_all_time_two_phase
    A B state hA hB hcontact
  have hcover : NoveltyCoverOn w N (g, state) times [] 2 := by
    refine ⟨[VectorCount.restrict N state,
      VectorCount.restrict N (flipAt state A.actionSwitch)], by simp, ?_⟩
    intro k hk
    obtain ⟨port, phase, hrun, hp⟩ := hphase k
    have hv : restrictedTonguesAt w N (g, state) k =
        VectorCount.restrict N phase := by
      simp [restrictedTonguesAt, tonguesAt, hrun]
    rw [hv]
    apply List.mem_append_right []
    rcases hp with rfl | rfl <;> simp
  have hc := noveltyCoverOn_distinct_count hcover hnd
  simpa using hc

/-- Direct distinct-vector count for the opposite intersecting stay/flip
orientation. -/
theorem manufactured_stay_then_flip_contact_distinct_le_two
    {w : Wiring} {N g e : Nat}
    (A : ManufacturedStayReflector w g e)
    (B : ManufacturedFlipReflector w e g)
    (state : Tongues)
    ...
    (hnd : (times.map
      (restrictedTonguesAt w N (g, state))).Nodup) :
    times.length ≤ 2 := by
  have hphase := manufactured_stay_then_flip_contact_all_time_two_phase
    A B state hA hB hcontact
  have hcover : NoveltyCoverOn w N (g, state) times [] 2 := by
    refine ⟨[VectorCount.restrict N state,
      VectorCount.restrict N (flipAt state B.actionSwitch)], by simp, ?_⟩
    intro k hk
    obtain ⟨port, phase, hrun, hp⟩ := hphase k
    have hv : restrictedTonguesAt w N (g, state) k =
        VectorCount.restrict N phase := by
