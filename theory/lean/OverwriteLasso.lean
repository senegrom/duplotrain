...
theorem pinTrajectory_add
  ...
  | succ n ih =>
      ...
      rw [hidx]
      show pinList (actions (start + n))
          (pinTrajectory actions t0 (start + n)) =
        pinList (pinBlock actions start (n + 1))
          (pinTrajectory actions t0 start)
      rw [ih]
      show pinList (actions (start + n))
          (pinList (pinBlock actions start n)
            (pinTrajectory actions t0 start)) =
        pinList (pinBlock actions start n ++ actions (start + n))
          (pinTrajectory actions t0 start)
      rw [pinList_append]
