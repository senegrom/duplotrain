...
/-!
# Protected repair costs six vectors

Every branch of the protected-repair analysis now has a constant tongue
count: the early backward exits and the final-mouth return cost three
(two-phase repair lead plus a settled or alternating tail), the facing
merge three, the changed stay splice three, the changed flip splice six,
and complete repair five.  Protected repair therefore exposes at most
**six** distinct restricted tongue vectors — the switch count `N` no
longer appears.
-/

namespace GeneralN

/-- Constant count for the state-changing backward contact, or the forward
certificate. -/
theorem ManufacturedReflector.protected_changed_constant_or_forward
    ...
    (∀ times : List Nat,
      (∀ k ∈ times, (stepN w k (g, B.activatedState)).isSome) →
  ...
  · obtain ⟨recorded, tail, hBsplit⟩ :=
    ...
    have happroachGroovedV : PassagesGrooved v approach := by
      rw [hflip]
      exact grooved_after_flip_other happroachGroovedU
        happroachForeign
...
/-- Constant count for the no-change backward contact, or the forward
certificate. -/
theorem ManufacturedReflector.protected_facing_constant_or_forward
  ...
  · have horientedEq : oriented = (fresh, p) := hsame
    ...
    have happroachGrooved : PassagesGrooved contact approach :=
      happroach.grooved_of_switchSimple happroachSimple
    have happroachReplay : PhysicalTrace w (g, contact) approach
        (p, contact) :=
...
/-- Protected-repair classification with every early exit carried as a
constant tongue count. -/
theorem manufactured_pair_protected_repair_quantitative_outcomes_constant
    {w : Wiring} {N g e : Nat}
  ...
  · obtain ⟨before, p, x, after, contact, other,
    ...
    · left
      ...
      exact ManufacturedReflector.return_change_facing_distinct_le_three
        A B hA hB hsplit hprefix hBcontact hp hreturn.1 hreturn.2
        times hlive hnd
    · obtain ⟨oldApproach, fresh, oldSuffix, oldU, oldV, path,
      ...
      rcases A.protected_facing_constant_or_forward B hA hB
  ...
  · rcases hrest with hchanged | hcomplete
    · obtain ⟨approach, p, x, suffix, u, v, path, old,
      ...
      rcases A.protected_changed_constant_or_forward B hA hB
...
/-- **Protected repair, constant count:** at most six distinct restricted
tongue vectors, for every switch count. -/
theorem manufactured_pair_protected_repair_distinct_le_six
    ...
    times.length ≤ 6 := by
  rcases manufactured_pair_protected_repair_quantitative_outcomes_constant
  ...
  · rcases hrest with hfacing | hrest
    ...
    · rcases hrest with hchanged | hcomplete
      · cases B with
        | stay R =>
            have hc := hchanged.stay_distinct_le_three hA hB
              times hlive hnd
            omega
        | flip R =>
            have hc := hchanged.flip_distinct_le_six hA hB
              times hnd
            omega
