...
/-- A changed-forward merge with its construction history supplies the exact
three-plus-empty decomposition expected by the known-edge sharp repair
certificate.  Thus no separate local runway residual remains in that
interface. -/
def ManufacturedReflector.ChangedForwardMerge.toKnownEdgeSharpRepairCertificate
    {w : Wiring} {g e N : Nat}
    {A : ManufacturedReflector w g e}
    {R : ManufacturedFlipReflector w e g}
    (hmerge : A.ChangedForwardMerge (.flip R))
    ...
    (hleadHistorical : ∀ j, j ≤ A.toSupported.travel →
      restrictedTonguesAt w N
        (g, (ManufacturedReflector.flip R).activatedState) j ∈ history)
    (times : List Nat) :
    KnownEdgeSharpRepairCertificate w N
      (g, (ManufacturedReflector.flip R).activatedState) times := {
  history := history
  pairTimes := times
  spliceTimes := []
  history_length := hhistory
  times_eq := by simp
  pair_cover :=
...
/-- Quantitative local consequence: an `N+1` historical construction prefix
followed by any changed-forward runway/candy residual contains at most `N+4`
pairwise-distinct restricted tongue vectors. -/
theorem ManufacturedReflector.ChangedForwardMerge.distinct_le_N_add_four
    {w : Wiring} {g e N : Nat}
    {A : ManufacturedReflector w g e}
    {R : ManufacturedFlipReflector w e g}
    (hmerge : A.ChangedForwardMerge (.flip R))
    ...
    times.length ≤ N + 4 := by
  have hcover := hmerge.runway_or_candy_absolute_three_novelty
    N history hleadHistorical times
