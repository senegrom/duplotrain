...
/-!
# Fourteen-N theta and sixteen-N reflector pair

With each capture bounded by `3*N`, a theta half costs `7*N`.  A one-sided
theta is two such halves (`14*N`).  In the mutual case, the reverse-capture
branch again uses a zero-lead return period; the other branches have lead plus
period at most `14*N`.

The only extra cost in the generic reflector-pair wrapper is one initial
reflector traversal of at most `2*N`, giving a uniform `16*N` lasso.
-/

namespace GeneralN

/-- One-sided flip/flip theta intersection closes within `14*N`. -/
theorem manufactured_one_sided_theta_within_fourteen
    {w : Wiring} {N g e : Nat}
    ...
    (hBA : (LocalAction.flip B.actionSwitch).Avoids
      [A.runway, A.candy]) :
    EventuallyPeriodicWithin w (g, state) (14 * N) := by
  obtain ⟨firstTravel, hfirstPos, hfirstLe, hfirst⟩ :=
    manufactured_theta_half_within_seven hN A B state hA hB hcontact
  ...
  obtain ⟨secondTravel, hsecondPos, hsecondLe, hsecond⟩ :=
    manufactured_theta_half_within_seven hN A B
...
/-- Mutual flip/flip intersection closes within `14*N`. -/
theorem manufactured_two_sided_theta_within_fourteen
    {w : Wiring} {N g e : Nat}
    (hN : ∀ p q, w.link p = some q →
      p < 3 * N ∧ q < 3 * N)
    (A : ManufacturedFlipReflector w g e)
  ...
  obtain ⟨lead, hleadPos, hleadLe, hlead⟩ :=
    manufactured_theta_half_within_seven hN A B state hA hB hAB
  ...
  have hreverseFault := manufactured_support_fault_dichotomy_within_three
    hN B A state hB hA hBA
  have hforwardFault := manufactured_support_fault_dichotomy_within_three
...
an `16*N` lasso. -/
theorem manufactured_pair_within_sixteen_mul_switches
    {w : Wiring} {N g e : Nat}
    (hN : ∀ p q, w.link p = some q →
      p < 3 * N ∧ q < 3 * N)
      ...
      | flip FB =>
          ...
          · by_cases hBA : (LocalAction.flip FB.actionSwitch).Avoids
            ...
            · have hcontactBA := contact_of_not_avoids_flip hBA
              ...
              have hlocal := manufactured_one_sided_theta_within_fourteen
          ...
          · have hcontactAB := contact_of_not_avoids_flip hAB
            by_cases hBA : (LocalAction.flip FB.actionSwitch).Avoids
                [FA.runway, FA.candy]
            · exact (manufactured_one_sided_theta_within_fourteen
                hN FA FB state hA hB hcontactAB hBA).weaken (by omega)
            · have hcontactBA := contact_of_not_avoids_flip hBA
              exact (manufactured_two_sided_theta_within_fourteen
