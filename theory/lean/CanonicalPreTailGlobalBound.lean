...
theorem canonical_supportWeight_fibre_bound
  ...
  | cons x rest =>
      ...
      have hloMemF' : lo ∈ ks.filter
          (fun k => decide (supportWeight m e r0 slots k = q)) := hloMemF
      have hhiMemF' : hi ∈ ks.filter
          (fun k => decide (supportWeight m e r0 slots k = q)) := hhiMemF
      have hloData := List.mem_filter.mp hloMemF'
      have hhiData := List.mem_filter.mp hhiMemF'
      ...
      have hweight : supportWeight m e r0 slots lo =
        ...
        (of_decide_eq_true hloData.2).trans
          (of_decide_eq_true hhiData.2).symm
