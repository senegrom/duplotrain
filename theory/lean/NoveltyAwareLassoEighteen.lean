...
/-! Overlap-aware count for the `16*N` protected repair. -/

namespace GeneralN

/-- Two exact manufactured journeys followed by a `16*N` protected-repair
lasso expose at most `18*N+2` distinct restricted tongue vectors. -/
theorem two_manufacturing_journeys_then_repair_distinct_le_eighteen
    {w : Wiring} {N e : Nat}
    ...
    (hlocal : EventuallyPeriodicWithin w (start.1, stateB) (16 * N))
    (times : List Nat)
