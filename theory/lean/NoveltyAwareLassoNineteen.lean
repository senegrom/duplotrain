...
/-! Overlap-aware count for the `17*N` protected repair. -/

namespace GeneralN

/-- Two exact manufactured journeys followed by a `17*N` protected-repair
lasso expose at most `19*N+2` distinct restricted tongue vectors. -/
theorem two_manufacturing_journeys_then_repair_distinct_le_nineteen
    {w : Wiring} {N e : Nat}
    ...
    (hlocal : EventuallyPeriodicWithin w (start.1, stateB) (17 * N))
    (times : List Nat)
    (hlive : ∀ k ∈ times, (stepN w k start).isSome)
    (hnd : (times.map (restrictedTonguesAt w N start)).Nodup) :
    times.length ≤ 19 * N + 2 := by
