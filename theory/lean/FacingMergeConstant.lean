...
/-!
# Constant count for the facing-forward merge

The facing merge's route-prefix lead is a protected repair prefix, hence
two-phase (`repair_prefix_two_phase`); from the contact onward the walk
shows only the `contact`/`alternate` pair.  The whole merge therefore
exposes at most **three** distinct restricted tongue vectors — no switch
count appears at all.
-/

namespace GeneralN

private theorem facingconst_twoPhase_concat
...
/-- **Facing-forward merge, constant count:** at most three distinct
...
theorem ManufacturedReflector.FacingForwardMerge.distinct_le_three
  ...
  have hcontactLoopPhase : ∀ d, d ≤ loopSteps → ∃ port phase,
      stepN w d (g, contact) = some (port, phase) ∧
        (phase = alternate ∨ phase = contact) := by
    intro d hd
    exact facingconst_twoPhase_concat hprefixContact.sound
  ...
  have hallPhase : ∀ d, ∃ port phase,
      ...
      · have haMouth : a = R.mouth := by omega
        ...
        have htoContactPhase : ∀ d,
            d ≤ prior.length + captureSteps → ∃ port phase,
              stepN w d (g, alternate) = some (port, phase) ∧
                (phase = alternate ∨ phase = contact) := by
          intro d hd
          exact facingconst_twoPhase_concat hpriorAlternate.sound
        ...
        have hperiodPhase : ∀ d, d ≤ period → ∃ port phase,
            stepN w d (g, alternate) = some (port, phase) ∧
              (phase = alternate ∨ phase = contact) := by
          intro d hd
          exact facingconst_twoPhase_concat htoContact htoContactPhase
      ...
      · have hbMouth : b = R.mouth := by omega
        ...
        have hperiodPhase : ∀ d, d ≤ loopSteps → ∃ port phase,
            stepN w d (g, alternate) = some (port, phase) ∧
              (phase = alternate ∨ phase = contact) := by
          intro d hd
          exact facingconst_twoPhase_concat hrepairPrefix.sound
    ...
    · have hforeign : ∀ passage ∈ before,
      ...
      have hperiodPhase : ∀ d, d ≤ loopSteps → ∃ port phase,
        ...
        exact facingconst_twoPhase_concat hprefixAlternate.sound
  ...
  have hBcontact : PathGrooves
      (ManufacturedReflector.flip R).toSupported.paths contact := hpaths
  have hphase := A.repair_prefix_two_phase
    (ManufacturedReflector.flip R) hA hBstart hprefix
    hbeforeSimple hbeforeRoute hBcontact
  have htail : ∀ (tailTimes : List Nat),
      (∀ k ∈ tailTimes, (stepN w k (p, contact)).isSome) →
      (tailTimes.map (restrictedTonguesAt w N (p, contact))).Nodup →
      tailTimes.length ≤ 2 := by
    intro tailTimes _htailLive htailNodup
    have hcover : NoveltyCoverOn w N (p, contact) tailTimes [] 2 := by
      refine ⟨[VectorCount.restrict N contact,
        VectorCount.restrict N alternate], by simp, ?_⟩
      intro k hk
      simp only [List.nil_append]
      have hmem : ∀ {port : Nat} {phase : Tongues},
          stepN w k (p, contact) = some (port, phase) →
          (phase = alternate ∨ phase = contact) →
          restrictedTonguesAt w N (p, contact) k ∈
            [VectorCount.restrict N contact,
              VectorCount.restrict N alternate] := by
        intro port phase hrun hphase
        have hvec : restrictedTonguesAt w N (p, contact) k =
            VectorCount.restrict N phase := by
          simp [restrictedTonguesAt, tonguesAt, hrun]
        rcases hphase with h | h
        · simp [hvec, h]
        · simp [hvec, h]
      by_cases hktail : k ≤ tailTravel
      · obtain ⟨port, phase, hrun, hphase⟩ :=
          htailContactPhase' k hktail
        exact hmem hrun hphase
      · let r := k - tailTravel
        have hkEq : k = tailTravel + r := by
          dsimp [r]
          omega
        obtain ⟨port, phase, hrun, hphase⟩ := hallPhase r
        have hrunGlobal : stepN w k (p, contact) =
            some (port, phase) := by
          rw [hkEq, stepN_add, htailContact]
          simpa using hrun
  ...
  exact two_phase_prefix_then_direct_tail_distinct_le_succ
    hprefix.sound hphase htail (by omega) times hlive hnd
