...
inductive CurveConnected (w : Wiring) (u : Tongues) : Nat → Nat → Prop
  | refl (p : Nat) : CurveConnected w u p p
  | cons {p q r : Nat} :
      CurveEdge w u p q →
      CurveConnected w u q r →
      CurveConnected w u p r

theorem CurveConnected.of_edge
    {w : Wiring} {u : Tongues} {p q : Nat}
    (h : CurveEdge w u p q) : CurveConnected w u p q :=
  .cons h (.refl q)
...
def RawCurveSelfJoin (w : Wiring)
  ...
  CurveConnected w before.2 before.1 (3*C)
...
theorem RawProductiveCurvePivot.self_or_connects_distinct_curves
    ...
    RawCurveSelfJoin w before C ∨
      (¬ CurveConnected w before.2 before.1 (3*C) ∧
       CurveConnected w after.2 before.1 (3*C)) := by
  by_cases hself : RawCurveSelfJoin w before C
  · exact Or.inl hself
  · right
    refine ⟨hself, ?_⟩
    apply CurveConnected.of_edge
