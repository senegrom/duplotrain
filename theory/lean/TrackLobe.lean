...
/-- The degenerate crossed case (`q = x`) is also a full reflector, but its
state map is the identity.  The apparent candy path collapses to a self-linked
edge by `same_exit_excursion_path_nil`. -/
theorem same_exit_revisit_full_reflector
    (w : Wiring) {start : Nat × Tongues}
    {runway path : List Passage}
    {p x e : Nat} {u₀ u : Tongues}
    (hrunway : PhysicalTrace w start runway (p, u₀))
    (hexcursion :
      PhysicalTrace w (p, u₀) ((p, x) :: path) (x, u))
    (hsimple : SwitchSimple (runway ++ (p, x) :: path))
    (hentry : w.link e = some start.1) :
    IsReflector w start.1 e (2 * runway.length + 2)
      (fun state =>
        PassagesGrooved state runway ∧ arrive state x = (p, state))
      (fun state => state) := by
  have hsimpleExcursion : SwitchSimple ((p, x) :: path) := by
    unfold SwitchSimple at hsimple ⊢
    simp only [List.map_append] at hsimple
    exact (List.nodup_append.mp hsimple).2.1
  have hpathNil := same_exit_excursion_path_nil hexcursion hsimpleExcursion
  subst path
  ...
  | nil =>
      cases hrunway
      have hmouth : w.link p = some e := w.symm _ _ hentry
      have hcore := self_edge_groove_isReflector w hself hmouth
      intro state hs
      obtain ⟨hstep, hnext⟩ := hcore state hs.2
      constructor
      · simpa using hstep
  ...
  | cons passage rest =>
      ...
      have hsandwich := sandwich_nonempty_reflector w
        ...
        hcore (fun _ hg => hg)
      have hlen :
          (((g, a) :: rest).length + 2 + ((g, a) :: rest).length) =
            2 * ((g, a) :: rest).length + 2 := by omega
      rw [hlen] at hsandwich
      simpa only [hstart] using hsandwich

/-- A state map is local when it is either the identity or flips exactly one
switch. -/
...
theorem first_revisit_cycle_or_reflector
    ...
    SettlesOnSimpleCycle w (q, u) ∨
      ∃ (travel : Nat) (S : Tongues → Prop) (τ : Tongues → Tongues),
        IsReflector w start.1 e travel S τ ∧ IsLocalSwitchMap τ := by
  have hsimpleExcursion : SwitchSimple ((p, x) :: path) := by
    unfold SwitchSimple at hsimple ⊢
    simp only [List.map_append] at hsimple
    exact (List.nodup_append.mp hsimple).2.1
  ...
  · subst q
    left
    exact hexcursion.simple_return_period hsimpleExcursion |> fun hp =>
      ⟨((p, x) :: path).length, u, by simp, hp, hp⟩
  · subst y
    by_cases hxq : x = q
    · subst q
      right
      refine ⟨2 * runway.length + 2,
        (fun state => PassagesGrooved state runway ∧
          arrive state x = (p, state)),
        (fun state => state), ?_, ?_⟩
      · exact same_exit_revisit_full_reflector w hrunway hexcursion
          hsimple hentry
      · exact Or.inl (fun _ => rfl)
    · right
      refine ⟨2 * runway.length + path.length + 2,
        (fun state => PassagesGrooved state runway ∧
          PassagesGrooved state path),
        (fun state => flipAt state (p / 3)), ?_, ?_⟩
      · exact crossed_revisit_full_reflector w hrunway hexcursion
          hsimple hrepeat hxq hentry
      · exact Or.inr ⟨p / 3, fun _ => rfl⟩
  · subst q
