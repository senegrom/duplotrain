...
private theorem stepN_some_of_positive_period
...
/-- Absolute-time novelty cover for a compatible runway splice.  Every
selected state before the splice is supplied by `history`; every state from
the splice onward is one of the four explicit tongue corners above. -/
theorem manufactured_suffix_explicit_lobe_absolute_four_novelty
  ...
  · intro j hj
    ...
    · let d := j - K
      ...
      have hperiod : stepN w period
        ...
        exact C.toSupported.paired_period L hOldAvoidsL hNewAvoidsC
          (flipAt state (mouth / 3)) hCNew hLNew
      have hlive := stepN_some_of_positive_period hperiodPos hperiod
        (d := d)
      ...
      refine ⟨tonguesAt w
        (outside, flipAt state (mouth / 3)) d, ?_, ?_⟩
      · exact manufactured_suffix_explicit_lobe_all_time_four_phase_tongues
          C state hCpaths hNewAvoidsC hActionsNe hentryBranch
          hentrySwitch hgrooved htrace hcrossed hCandyForeignNew
          hCandyForeignOld hLobe hmouthLink d
      · simp [restrictedTonguesAt, hshift]

/-- Absolute-time novelty cover for the intersecting-action runway splice.
The theta construction is pointwise four-phase, so the historical prefix and
the complete residual trajectory together require at most four fresh vectors. -/
theorem manufactured_flip_arbitrary_lobe_absolute_four_novelty
    {w : Wiring} {outside mouth entry returnPort N : Nat}
    (C : ManufacturedFlipReflector w outside mouth)
    (state : Tongues)
    (hCpaths : PathGrooves C.toSupported.paths state)
  ...
  · intro j hj
    ...
    · let d := j - K
      have hjEq : j = K + d := by
        dsimp [d]
        omega
      have hlive := stepN_some_of_positive_period hperiodPositive hperiod
        (d := d)
      have hshift := tonguesAt_add_of_reaches hreach hlive
      rw [← hjEq] at hshift
      apply List.mem_append_right history
...
/-- Full changed-forward reduction with the runway branch made pointwise.

* A candy contact is returned exactly as in the older splice theorem.
* A runway contact whose new candy avoids the shortened suffix action has an
  absolute four-vector cover.
* The sole remaining runway case is named explicitly: the new candy itself
  visits the shortened suffix reflector's action switch.

Thus no route-length quantity remains in the compatible runway branch. -/
theorem ManufacturedReflector.ChangedForwardMerge.runway_splice_reduction
    {w : Wiring} {g e : Nat}
    ...
    (∃ (outside mouth : Nat) (C : ManufacturedFlipReflector w outside mouth)
        (candy : List Passage),
      ∃ passage ∈ candy,
        passageSwitch passage = C.actionSwitch) ∨
    FourNoveltyCover w N
  ...
  obtain ⟨entry, mouth, returnPort, outside, oldPrefix, oldTail,
      ...
      hrouteSplit, _hOldTail, _hApproach, _hApproachGrooved,
  ...
  · obtain ⟨before, after, hrunwaySplit⟩ :=
    ...
    by_cases hcontact : ∃ passage ∈ candy,
        passageSwitch passage = C.actionSwitch
    · exact Or.inr (Or.inl ⟨outside, mouth, C, candy, hcontact⟩)
    · right
      right
      ...
      apply manufactured_suffix_explicit_lobe_absolute_four_novelty
        C state hCpaths hNewAvoidsC hActionsNe hentryBranch
...
/-- **Closed changed-forward novelty theorem.**  Candy splices use their
sharper one-vector theorem.  A runway splice uses the compatible pair theorem
when the actions are disjoint and the pointwise theta theorem when the new
candy meets the shortened suffix action.  Hence every changed-forward merge,
including the exact runway residual left by
`runway_or_candy_absolute_one_novelty`, has an absolute four-vector cover. -/
theorem ManufacturedReflector.ChangedForwardMerge.runway_or_candy_absolute_four_novelty
    {w : Wiring} {g e : Nat}
    {A : ManufacturedReflector w g e}
    {R : ManufacturedFlipReflector w e g}
    (hmerge : A.ChangedForwardMerge (.flip R))
  ...
  · obtain ⟨before, after, hrunwaySplit⟩ :=
    ...
    by_cases hcontact : ∃ passage ∈ candy,
        passageSwitch passage = C.actionSwitch
    · apply manufactured_flip_arbitrary_lobe_absolute_four_novelty
        C state hCpaths hNewAvoidsC hentryBranch hentrySwitch
        hfullGrooved hfullTrace hcrossed hCandyForeignNew hLobe
        hmouthLink hcontact hreach times history
      intro j hj hjLead
      apply hleadHistorical j
      rw [hleadLen] at hjLead
      omega
