...
/-! Overlap-aware count for the sharpened `21*N` repair lasso. -/

namespace GeneralN

/-- Two exact manufactured journeys followed by the sharpened `21*N` repair
lasso expose at most `23*N+2` distinct restricted tongue vectors. -/
theorem two_manufacturing_journeys_then_repair_distinct_le_twenty_three
    {w : Wiring} {N e : Nat}
    ...
    (hlocal : EventuallyPeriodicWithin w (start.1, stateB) (21 * N))
    (times : List Nat)
    (hlive : ∀ k ∈ times, (stepN w k start).isSome)
    (hnd : (times.map (restrictedTonguesAt w N start)).Nodup) :
    times.length ≤ 23 * N + 2 := by
