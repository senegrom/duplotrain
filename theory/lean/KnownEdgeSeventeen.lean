...
import NoveltyAwareLassoSeventeen

/-! Known-edge coefficient-17 linear bound. -/

namespace GeneralN

/-- Known-edge long-run form of the `17*N+4` bound. -/
theorem known_edge_long_run_distinct_le_seventeen
    {w : Wiring} {N e : Nat}
  ...
  · obtain ⟨A, B, stateA, stateB,
    ...
    have hlocal := manufactured_pair_protected_repair_within_fifteen_succ_two
      hN A B hAatBase hBatActivated
    rw [← hactivatedB] at hlocal
    exact two_manufacturing_journeys_then_repair_distinct_le_seventeen
