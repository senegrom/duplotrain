...
/-- First-writer charging plus the four-event theorem gives an `N + 5`
bound for every finite set of distinct samples from a compatible-pair
trajectory.  This is a raw `stepN` theorem; no eventual-periodicity or echo
machine premise is used. -/
theorem first_turnaround_then_compatible_pair_distinct_le_N_add_five
    {w : Wiring} {h g e N : Nat}
    (P : ManufacturedReflector w h g)
    (A : ManufacturedReflector w g e)
    (B : ManufacturedReflector w e g)
    ...
    (hreach : stepN w
      (P.exploration.length + P.runway.length + 1)
      (h, P.baseState) = some (g, state))
    (H : Nat) (times : List Nat)
    (htimes : forall k, k ∈ times -> k ≤ H)
    (hnd : (times.map
      (restrictedTonguesAt w N (h, P.baseState))).Nodup) :
    times.length ≤ N + 5 := by
  have hfour :=
    first_turnaround_then_compatible_pair_repeatedWriterNovelty_le_four
      P A B state hN hPpaths hA hB hAB hBA hreach H
  have hcount := distinct_samples_le_of_repeated_writer_novelty
    w N hN (h, P.baseState) H 4 hfour times htimes hnd
  omega

/-- The direct-lobe second-turnaround branch is automatically compatible,
so it inherits the improved four-event charge without an additional
compatibility hypothesis. -/
theorem first_turnaround_then_direct_pair_repeatedWriterNovelty_le_four
  ...
  exact first_turnaround_then_compatible_pair_repeatedWriterNovelty_le_four
