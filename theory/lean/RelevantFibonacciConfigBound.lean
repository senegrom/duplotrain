...
import RelevantFibonacciNBound

/-!
# Direct Fibonacci count for finite echo configurations

A finite configuration is `(current entry, register snapshot)`.  Partition by
entry exactly as in `EchoConfigCount`; each fibre now uses the constructible
Fibonacci snapshot theorem directly instead of eighth-power aggregation.
-/

namespace Echo

private theorem fibonacci_config_sum_le
    (xs : List Nat) (B : Nat)
    (h : ∀ x ∈ xs, x ≤ B) :
...
/-- Distinct finite configurations cost one Fibonacci snapshot fibre per
possible current entry. -/
theorem relevantFrame_config_fibonacci_bound
    (hrun : IsRun m e r0)
    ...
    ks.length ≤ entries.length *
      ((2*N + 1) * fibBalancedCapacity N + 4) := by
  ...
  have heach : ∀ s ∈ sizes,
      s ≤ (2*N + 1) * fibBalancedCapacity N + 4 := by
    intro s hs
    ...
    have hbound := relevant_finiteFrame_N_fibonacci_bound
  ...
  have hagg := fibonacci_config_sum_le sizes
    ((2*N + 1) * fibBalancedCapacity N + 4) heach
  have hlen : sizes.length = entries.length := by
    simp [sizes, entryFibreSizes]
  rw [hsum, hlen] at hagg
  exact hagg

/-- At most `2*N` possible current entries. -/
theorem relevantFrame_config_atMost_N_fibonacci_bound
    (hrun : IsRun m e r0)
    (hr0 : ∀ c, m.cellOf (r0 c) = c)
    (N globalLo globalHi : Nat)
    (cells slots entries ks : List Nat)
    ...
    ks.length ≤ (2*N) *
      ((2*N + 1) * fibBalancedCapacity N + 4) := by
  have hbase := relevantFrame_config_fibonacci_bound
