...
/-!
# Two fresh vectors when two Gray corners are historical

A protected changed-forward flip lead already records both the entering
alternate state and the unflipped contact state.  The runway/candy tail has
four Gray corners, so only the other two corners can be globally fresh.
-/
