import TrackQuantitativeRepairNineteen
import NoveltyAwareLassoTwentyOne

/-! Known-edge coefficient-21 linear bound. -/

namespace GeneralN

/-- Known-edge long-run form of the `21*N+2` bound. -/
theorem known_edge_long_run_distinct_le_twenty_one
    {w : Wiring} {N e : Nat}
  ...
  · obtain ⟨A, B, stateA, stateB,
    ...
    have hlocal := manufactured_pair_protected_repair_within_nineteen
      hN A B hAatBase hBatActivated
    rw [← hactivatedB] at hlocal
    exact two_manufacturing_journeys_then_repair_distinct_le_twenty_one
