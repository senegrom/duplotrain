...
/-! Overlap-aware count for the sharpened `19*N` repair lasso. -/

namespace GeneralN

/-- Two exact manufactured journeys followed by the sharpened `19*N` repair
lasso expose at most `21*N+2` distinct restricted tongue vectors. -/
theorem two_manufacturing_journeys_then_repair_distinct_le_twenty_one
    {w : Wiring} {N e : Nat}
    ...
    (hlocal : EventuallyPeriodicWithin w (start.1, stateB) (19 * N))
    (times : List Nat)
