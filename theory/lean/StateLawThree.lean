...
/-! Unconditional entry-free coefficient-3 linear bound. -/

namespace GeneralN

private theorem three_nodup_of_map_nodup
...
private theorem three_nodup_map_filter
...
private theorem three_nodup_filter_nat (p : Nat → Bool) :
...
private theorem three_zero_positive_partition :
    ∀ xs : List Nat,
...
/-- **Unconditional coefficient-three state bound:** every live `N`-switch
run has at most `3*N+8` pairwise-distinct restricted tongue vectors. -/
theorem state_law_linear_three
    (w : Wiring) (N : Nat)
    ...
    times.length ≤ 3 * N + 8 := by
  have htimesNodup : times.Nodup :=
    three_nodup_of_map_nodup
      ...
      | some middle =>
          ...
          | some entry =>
              ...
              have hpositiveNodup :
                ...
                dsimp [positive]
                exact three_nodup_map_filter _ hnd
              have hshiftedNodup :
              ...
              have hshiftedLive : ∀ d ∈ shifted,
                ...
                exact hkLive
              have hshiftedBound : shifted.length ≤ 3 * N + 7 :=
                known_edge_long_run_distinct_le_three
              ...
              have hzeroBound :
                ...
                three_zero_filter_length_le_one htimesNodup
              have hpartition := three_zero_positive_partition times
