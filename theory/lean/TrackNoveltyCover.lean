...
bounds distinct live samples by the number of preceding times. The pointwise
form of the contact-retrace theorem comes first: after a contact step, every
intermediate configuration on the reverse traversal carries the contact vector.
-/
