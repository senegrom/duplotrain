import KnownEdgeEighteen

/-! Unconditional entry-free coefficient-18 linear bound. -/

namespace GeneralN

private theorem eighteen_nodup_of_map_nodup
...
private theorem eighteen_nodup_map_filter
...
private theorem eighteen_nodup_filter_nat (p : Nat → Bool) :
...
private theorem eighteen_zero_positive_partition :
    ∀ xs : List Nat,
...
has at most `18*N+3` pairwise-distinct restricted tongue vectors. -/
theorem state_law_linear_eighteen
    (w : Wiring) (N : Nat)
    ...
    times.length ≤ 18 * N + 3 := by
  by_cases hNtwo : 2 ≤ N
  · have htimesNodup : times.Nodup :=
      eighteen_nodup_of_map_nodup
        ...
        | some middle =>
            ...
            | some entry =>
                ...
                have hpositiveNodup :
                  ...
                  dsimp [positive]
                  exact eighteen_nodup_map_filter _ hnd
                have hshiftedNodup :
                ...
                have hshiftedLive : ∀ d ∈ shifted,
                  ...
                  exact hkLive
                have hshiftedBound : shifted.length ≤ 18 * N + 2 :=
                  known_edge_long_run_distinct_le_eighteen
                    hN hNtwo hlong' hlink shifted
                    hshiftedLive hshiftedNodup
                have hpositiveLength : positive.length = shifted.length := by
                  simp [shifted]
                have hzeroBound :
                    (times.filter (fun k => decide (k = 0))).length ≤ 1 :=
                  eighteen_zero_filter_length_le_one htimesNodup
                have hpartition := eighteen_zero_positive_partition times
