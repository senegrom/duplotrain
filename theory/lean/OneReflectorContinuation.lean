...
/-- Same-exit first-revisit specialization.  The pointwise cycle theorem
supplies the one-vector positive tail, improving the old independent
`N+2` cycle-tail charge to one settled vector. -/
theorem same_exit_cycle_after_simple_lead_distinct_le_N_add_three
    {w : Wiring} {N g e p x q : Nat}
    (hN : ∀ a b, w.link a = some b →
      a < 3 * N ∧ b < 3 * N)
    (A : ManufacturedReflector w g e)
    (hA : PathGrooves A.toSupported.paths A.activatedState)
    {lead : List Passage} {u₀ u v : Tongues}
    (hlead : PhysicalTrace w
      (e, A.activatedState) lead (q, u))
    (hleadSimple : SwitchSimple lead)
    (hend : PathGrooves A.toSupported.paths u)
    {rest : List Passage}
    (hcycle : PhysicalTrace w
      (p, u₀) ((p, x) :: rest) (q, u))
    (hcycleSimple : SwitchSimple ((p, x) :: rest))
    ...
    (hnd : (times.map
      (restrictedTonguesAt w N (g, A.baseState))).Nodup) :
    times.length ≤ N + 3 := by
  have htail : ∀ d, 0 < d → ∃ port,
      stepN w d (q, u) = some (port, v) :=
    hcycle.simple_same_exit_cycle_all_positive
      hcycleSimple hnext
  exact simple_lead_one_vector_tail_distinct_le_N_add_three
    hN A hA hlead hleadSimple hend htail times hlive hnd

/-- Requested `N+6` interface for the terminating support-disjoint branch.
The established estimate is the sharper `N+2`. -/
theorem foreign_simple_fall_distinct_le_N_add_six
    {w : Wiring} {N g e : Nat}
    (hN : ∀ p q, w.link p = some q →
      p < 3 * N ∧ q < 3 * N)
    (A : ManufacturedReflector w g e)
    ...
    (hnd : (times.map
      (restrictedTonguesAt w N (g, A.baseState))).Nodup) :
    times.length ≤ N + 6 := by
  have hsharp := foreign_simple_fall_distinct_le_N_add_two
    hN A hA htrace hsimple hforeign hfall times hlive hnd
  omega
