...
private theorem finite_sum_map_zero {α : Type} :
...
theorem finiteFibreSizes_sum
  ...
  | nil =>
      intro _ _
      show (values.map (fun q => finiteFibreSize f q [])).sum = 0
      have h : ∀ q ∈ values, finiteFibreSize f q [] = 0 := by
        intro q _
        rfl
      rw [List.map_congr_left h]
      exact finite_sum_map_zero values
  | cons k rest ih =>
      intro hnd hcover
      ...
      have hfold : (values.map
          (fun q => finiteFibreSize f q rest)).sum = rest.length :=
        ih hnd hrest
      rw [hfold]
