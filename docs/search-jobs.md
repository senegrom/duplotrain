...
allowance to 16, 32 and then 50 distinct exact-layout identities, retaining the
current search's suspended traversal. **Search harder** raises both the node
...
**Pause at next checkpoint** stops advancing the engine without destroying its
worker or undo history. **Stop with results found** does the same and publishes
accepted suggestions. **Resume** continues the stored job. A preview arriving
while the search is running is not directly applicable: the engine must first
publish its candidate revision. Applying a suggestion is one normal, undoable
edit and releases the old job.
...
substitute for the width-inclusive room check below.
...
without moving or discarding pieces.
...
Optional restrictions and ranking are project preferences. Existing layout,
session and autosave formats are unchanged; old projects without these optional
preferences still open.

## Closing all gaps

**Close all gaps** accepts two to ten open ends, exact joins and non-reversing
...
`solve_steps` is the cooperative form of the existing exact solver. A
`SearchLimits` instance supplies mutable node, result and completion-depth
bounds. It yields progress, accepted-solution and limit events. Raising a bound
lets the caller resume the suspended DFS, including its inventory, collision
backtracking and reverse tables. Closing an iterator releases its workspace.
The existing synchronous `solve` drains this iterator and keeps its API and
search semantics. Fresh-loop enumeration retains its configured depth behavior;
raising a piece bound on a continuation is supported for completion mode.

Interactive single-pair jobs also retain the ordinary/bridge/full-inventory
search stages and alternating endpoints. Stage node budgets are shared across
...
**Find best start/settings** can consider the selected inward start or every
valid inward start, in either case trying all initial switch assignments within
the chosen budget. Choose maximum lifetime coverage or maximum repeating-cycle
coverage. The report identifies the checked scope, completed runs, required
runs, best witness and a counterexample where applicable. **Load best route**
and **Load counterexample** fill the existing start/switch controls and run the
normal trace without changing track geometry.
